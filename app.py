...
# Import the full Neural Enhancement Pipeline
...
def enhance(filename):
    ...
    if request.method == 'POST':
        ...
        # Enhancement parameters based on the strength slider
        enhancement_params = {
            # General parameters that work with all image types
            'detail_strength': strength,
            'color_strength': strength,
            'frequency_strength': strength,
            'use_denoising': True,
            'use_super_resolution': strength > 0.3,  # Only use super-resolution for higher strength settings
            
            # Enable specific enhancements based on strength
            'use_semantic': strength > 0.4,
            'use_depth': strength > 0.5,
            'use_object_enhancement': True,
            
            # Style transfer is optional and only for higher strengths
            'use_style_transfer': strength > 0.7,
            'style_strength': max(0, (strength - 0.7) * 3),  # Scale from 0 to 0.9
        }
...
def api_enhance():
    ...
    if file and allowed_file(file.filename):
        ...
        enhancement_params = {
            ...
            'frequency_strength': strength,
            'use_denoising': True,
            'use_super_resolution': strength > 0.3,
            'use_semantic': strength > 0.4,
            'use_depth': strength > 0.5,
            'use_object_enhancement': True,
            'use_style_transfer': strength > 0.7,
            'style_strength': max(0, (strength - 0.7) * 3),
        }
